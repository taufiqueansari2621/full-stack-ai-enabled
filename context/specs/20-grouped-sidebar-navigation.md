...
The global Forge sidebar presents the large workspace map as a primary Home
destination plus clear Learn, Practice, Build, Prepare, and Personal groups
while preserving every route, active state, due-review count, learner status,
weekly goal, mobile drawer, and focused learning behavior.

## Requirements

- Keep one typed source of truth for navigation labels, icons, and route IDs.
- Render Home first, then semantic labelled groups in this order: Learn,
  Practice, Build, Prepare, Personal, then Help.
